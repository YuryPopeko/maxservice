...
function toggleTab(button) {
	...
	var dataTab = button.dataset.tab.split(':');
	...
	if (button.classList.contains('open')) {
		...
		document.querySelectorAll('[data-tab^=' + group + '].open').forEach(function (item) {
...
function openModal(button) {
	...
	var modalWindow = document.querySelector('.modal-' + button.dataset.modal);
...
function initMap() {
	...
	if (mapToggle) {
		mapToggle.addEventListener('click', function (e) {
			...
			e = e || window.event;
			var target = e.target;
			var btn = target.closest('[data-city]');
...
function toggleAccordion(accordionBtn) {
	...
	var accordionContent = accordionBtn.parentElement.querySelector('.accordion__content');
...
(function () {
	...
	var accordionBtnActive = document.querySelectorAll('.accordion__btn_active');
	if (!accordionBtnActive.length) return;

	accordionBtnActive.forEach(function (item) {
		var accordionContent = item.parentElement.querySelector('.accordion__content');
		accordionContent.style.maxHeight = accordionContent.scrollHeight + "px";
	});
})();
